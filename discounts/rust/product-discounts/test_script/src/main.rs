...
fn targets(
    ...
    merchandise_lines
        ...
        .filter_map(|line| match line.variant {
            Some(ref variant) if !excluded_variant_gids.contains(&variant.id) => {
                Some(output::Target {
...
fn build_output(value: f64, targets: Vec<output::Target>) -> output::Variables {
    output::Variables {
        result: output::HandleResult {
            discounts: vec![output::Discount {
                ...
                value: output::Value {
                    percentage: None,
                    fixedAmount: Some(output::FixedAmount {
                        value,
